...
from utils.preprocessors import Preprocessor, is_spatial_action, stack_ndarray_dicts


class MiniGame:
    def __init__(self, agent, envs, summary_writer, train=True, n_steps=8, discount=0.99):
        """
        Args:
    ...
    def summarize_episode(self, timestep):
    ...
    def run_batch(self, train_summary=False):
        ...
        for n in range(self.n_steps):
            ...
            for t in obs_raw:
                if t.last():
                    score = self.summarize_episode(t)
        ...
        if self.train:
            return self.agent.train(
                obs, actions, returns, advs, summary=train_summary)
